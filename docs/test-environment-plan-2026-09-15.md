...
### Zoho Outreach Checkpoint: `2026.09.15.3-TEST`
...
- The complete `2026.09.15.3-TEST` source remains locked to the staging
  spreadsheet and blocks queued and LIVE sends at both menu and send-function
  levels.
- Every template now includes a small public Sturgeon Spirits logo linked to
  the company website, plus a text-link fallback. The website and logo URLs are
  maintained in `Email Editor!B14:B15`, not hardcoded in the script.
...
pale-yellow `Email Editor!B12` cell and flows into `Campaign Settings`
